    ...
    if is_owner:
        if resource_type and resource_type.lower() in DOWNLOADABLE_RESOURCES:
            if resource_subtype and resource_subtype.lower() in safe_list(DATA_EDITABLE_RESOURCES_SUBTYPES + DATA_STYLABLE_RESOURCES_SUBTYPES):
                return safe_list(LAYER_ADMIN_PERMISSIONS + OWNER_PERMISSIONS + DOWNLOAD_PERMISSIONS)
            else:
        ...
        if resource_type and resource_type.lower() in DOWNLOADABLE_RESOURCES:
            if resource_subtype and resource_subtype.lower() in safe_list(DATA_EDITABLE_RESOURCES_SUBTYPES + DATA_STYLABLE_RESOURCES_SUBTYPES):
                return safe_list(LAYER_ADMIN_PERMISSIONS + VIEW_PERMISSIONS + EDIT_PERMISSIONS + DOWNLOAD_PERMISSIONS)
            else:
        ...
        if resource_type and resource_type.lower() in DOWNLOADABLE_RESOURCES:
            if resource_subtype and resource_subtype.lower() in safe_list(DATA_EDITABLE_RESOURCES_SUBTYPES + DATA_STYLABLE_RESOURCES_SUBTYPES):
                return safe_list(LAYER_ADMIN_PERMISSIONS + VIEW_PERMISSIONS + ADMIN_PERMISSIONS + DOWNLOAD_PERMISSIONS)
            else:
    ...
    if _perm:
        for _p in COMPACT_RIGHT_MODES:
            if (_p[1] not in [DOWNLOAD_RIGHTS] + LAYER_ADMIN_PERMISSIONS or
                    ...
                    _p[1] in LAYER_ADMIN_PERMISSIONS and any(__p in DATA_EDITABLE_RESOURCES_SUBTYPES + DATA_STYLABLE_RESOURCES_SUBTYPES for __p in perms)):
