  ...
  /*
    ...
    */
  dataset.controller('dataset_controller', function($scope, $rootScope, $location, $http, $uibModal) {
      ...
      $scope.post_file = function (dataset_id) {
          ...
          var handlerAllowedExtMsg = (ext) => {
            return `
              <div id="allowedFileExtToast" class="position-fixed bottom-0 right-0 p-3" style="z-index: 99999; right: 0; bottom: 0;">
                <div class="toast-message alert-warning align-items-center" role="alert" aria-live="assertive" aria-atomic="true">
                  <div class="toast-header">
                    <strong class="mr-auto">File Format Not Allowed</strong>
                    <small class="text-muted"></small>
                    <button type="button" class="ml-2 mb-1 close" onclick="document.getElementById('allowedFileExtToast').remove()" aria-label="Close">
                      <span aria-hidden="true">&times;</span>
                    </button>
                  </div>
                  <div class="toast-body">
                    <span class="font-lg-1">Your file format/extension is not supported by system: `+ext+`. Try archive your file or contact to admin for allowing your file type.</span>
                  </div>
                </div>
              </div>
            `
          ...
          // checking file extension if it allows by the system/administrator
          var CHECK_FILE_EXTENSION = siteUrl + "api/v2/datasets/get-allowed-ext"
          // checking allowed max size of uploaded file
          ...
          if (file || file_url) {
              if (file) {
                  ...
                  $http.get(CHECK_FILE_EXTENSION).then(successCallbackExt);
              }
              if (file_url) {
                  var handlerValidUrl = (ext) => {
                    return `
                      <div id="handlerValidUrlToast" class="position-fixed bottom-0 right-0 p-3" style="z-index: 99999; right: 0; bottom: 0;">
                        <div class="toast-message alert-warning align-items-center" role="alert" aria-live="assertive" aria-atomic="true">
                          <div class="toast-header">
                            <strong class="mr-auto">Provide Valid URL</strong>
                            <small class="text-muted"></small>
                            <button type="button" class="ml-2 mb-1 close" onclick="document.getElementById('handlerValidUrlToast').remove()" aria-label="Close">
                              <span aria-hidden="true">&times;</span>
                            </button>
                          </div>
                          <div class="toast-body">
                            <span class="font-lg-1">Seems your link URL is not valid. Please provide valid link starting from http:// or https://.</span>
                          </div>
                        </div>
                      </div>
                    `
                  ...
                  if (isValidHttpUrl(file_url)) {
                      ...
                      $(document.body).append(handlerValidUrl());
                      $('#submit').prop("disabled", false);
                  }
              }
          } else {
          ...
          function successCallbackExt(data) {
              var allowed_extensions = data.data["objects"];
              var extension = file.substring(file.lastIndexOf('.') + 1);
              if (!allowed_extensions.includes(extension)) {
                  $(document.body).append(handlerAllowedExtMsg(extension));
                  setTimeout(function() {
                    $('#allowedFileExtToast').remove();
                  }, 8000)
                  // remove event handler and selected files
      ...
      $scope.upload_datasets = function($event) {
          ...
          $('#drop-zone').addClass('drop-selected');
  ...
  dataset.upload_dataset_file = function($http, $rootScope, file, dataset_id, drag = false) {
      ...
      return {
          ...
          link: function (scope, elem) {
              ...
              elem.on('drop', function(evt) {
                  ...
                  var uploader = dataset.upload_dataset_file($http, $rootScope, files, dataset_id, true);
              });
...
document.getElementById('top').setAttribute('ng-controller', "dataset_controller");
