...
class DatasetTimeSerieForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        _choices = [(None, '-----')] + [(_a.pk, _a.attribute) for _a in kwargs.get('instance').attributes if _a.attribute_type in ['xsd:dateTime']]
        self.base_fields.get('attribute').choices = _choices
        self.base_fields.get('end_attribute').choices = _choices
        super().__init__(*args, **kwargs)
        for field in self.fields:
            self.fields[field].widget.attrs.update({'class': 'col'})            
