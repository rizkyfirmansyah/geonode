...
from geonode.datasets import DatasetsAppConfig
...
class DocumentsTest(GeoNodeBaseTestSupport):
    ...
    def test_create_document_url_view(self):
        ...
        response = self.client.post(reverse('dataset_upload'), data=form_data)
    ...
    def test_upload_document_form(self):
        ...
        self.assertTrue(
            'permissions' in DatasetCreateForm(
        ...
        file_data = {
            'doc_file': SimpleUploadedFile(
                ...
                self.imgfile.read(),
                'image/gif')}
        form = DatasetCreateForm(form_data, file_data)
        self.assertTrue(form.is_valid())
    ...
    def test_document_details(self):
        ...
        response = self.client.get(reverse('dataset_detail', args=(str(d.id),)))
        self.assertEqual(response.status_code, 200)

    def test_dataset_metadata_details(self):
        d = File.objects.all().first()
        d.set_default_permissions()

        response = self.client.get(reverse('dataset_metadata_detail', args=(str(d.id),)))
        self.assertEqual(response.status_code, 200)
    ...
    def test_moderated_upload(self):
        ...
        with self.settings(ADMIN_MODERATE_UPLOADS=True):
            ...
            resp = self.client.get(
                reverse('dataset_detail', args=(_d.id,)))
            # Forbidden
...
class DocumentsNotificationsTestCase(NotificationsTestsHelper):
    ...
    def setUp(self):
        ...
        self.u.save()
        self.setup_notifications_for(DatasetsAppConfig.NOTIFICATIONS, self.u)
        self.norman = get_user_model().objects.get(username='norman')
        self.norman.email = 'norman@email.com'
        self.norman.is_active = True
        self.norman.save()
        self.setup_notifications_for(DatasetsAppConfig.NOTIFICATIONS, self.norman)
...
class DocumentResourceLinkTestCase(GeoNodeBaseTestSupport):
    ...
    def test_create_document_with_links(self):
        ...
        d = File.objects.create(
            doc_file=f,
            owner=superuser,
            title='theimg'
        )

        self.assertEqual(File.objects.get(pk=d.id).title, 'theimg')
...
class DocumentViewTestCase(GeoNodeBaseTestSupport):
    ...
    def setUp(self):
        ...
        f = SimpleUploadedFile(
            ...
            self.imgfile.read(),
            'image/gif')
        self.test_doc = File.objects.create(doc_file=f, owner=self.not_admin, title='test', is_approved=True)
        self.perm_spec = {"users": {"AnonymousUser": []}}
        self.dock_link_url = reverse('dataset_link', args=(self.test_doc.pk,))
    ...
    def test_that_keyword_multiselect_is_disabled_for_non_admin_users(self):
        ...
        url = reverse('dataset_metadata', args=(self.test_doc.pk,))
