...
def csw_render_extra_format_txt(request, layeruuid, resname):
    ...
    content += f"publication date{s}{fst(resource.date)}{sc}"
    content += f"date type{s}{fst(resource.date_type)}{sc}"
    content += f"distribution date{s}{fst(resource.date_distribution)}{sc}"
    ...
    content += f"category{s}"
    for cat in resource.category.all():
        content += f"{fst(cat.gn_description)},"
    content = content[:-1]
    content += sc
