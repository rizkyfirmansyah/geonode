...
def facets(context):
    ...
    keywords_filter = request.GET.getlist('7e31fcb', None)
    category_filter = request.GET.getlist('f4e493d', None)
    data_type_filter = request.GET.getlist('182243e', None)
    ...
    if facet_type == 'catalogue':
        ...
        datasets = Dataset.objects.filter(title__icontains=title_filter)
        if data_type_filter:
            datasets = datasets.filter(data_type__identifier__in=data_type_filter)
        if link_filter:
            datasets = datasets.filter(link__extension__in=link_filter)
...
def get_visibile_resources(user):
    categories = get_objects_for_user(user, 'view_resourcebase', klass=ResourceBase, any_perm=False)\
        .filter(category__isnull=False).values('category__gn_description',
                                               'category__fa_class', 'category__svg', 'category__title', 'category__identifier')\
...
def display_edit_request_button(resource, user, perms):
    def _has_owner_his_permissions():
        ...
        if resource.resource_type in ['layer', 'dataset']:
            '''
            The download resource permission should be available only
            if the resource is a Layer or Datasets. You cant download maps
            '''
