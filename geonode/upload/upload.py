...
from ..layers.utils import get_valid_layer_name, is_vector
    ...
    if import_session:
        ...
        with AcquireLock(lock_id, blocking=True) as lock:
            if lock.acquire() is True:
                ...
                if saved_layer:
                    name = saved_layer.get_real_instance().name
                ...
                if saved_layer:
                    ...
                    _log(f'Django record for [{saved_layer.get_real_instance().name}] already exists, updating with vals: {_vals}')
                ...
                if sld_file:
                    ...
                    if upload_session.base_file.archive:
                        ...
                        try:
                            ...
                            logger.exception(e)
                            raise GeneralUploadException(detail=_('Error uploading Layer') + str(e))
                    sld_uploaded = True
                else:
                ...
                if upload_session.append_to_mosaic_opts:
                    ...
                    saved_layer_filter = Layer.objects.filter(
                    ...
                    elif saved_layer_filter.count() == 1:
                        saved_layer = saved_layer_filter.get()
                        created = False
                    else:
                    ...
                    # The layer is a standard one, no mosaic options enabled...
                    saved_layer_filter = Layer.objects.filter(
                        store=_vals.get('store'),
                        workspace=_vals.get('workspace'),
                        name=_vals.get('name'))
                ...
                saved_layer.set_processing_state(enumerations.STATE_RUNNING)
                saved_layer.set_dirty_state()
                _log(f" -- Finalizing Upload for {saved_layer}... {saved_layer.state}")
                ...
                try:
                    with transaction.atomic():
                        resource_manager.set_permissions(
                            None, instance=saved_layer, permissions=permissions, created=created)
                        resource_manager.update(
                            None, instance=saved_layer, xml_file=xml_file, metadata_uploaded=metadata_uploaded)
                        resource_manager.exec(
                            'set_style', None, instance=saved_layer, sld_uploaded=sld_uploaded, sld_file=sld_file, tempdir=upload_session.tempdir)
                        resource_manager.exec(
                            'set_time_info', None, instance=saved_layer, time_info=upload_session.time_info)
                        resource_manager.set_thumbnail(
                            None, instance=saved_layer)

                        Upload.objects.filter(resource=saved_layer).update(complete=True)
                        [u.set_processing_state(enumerations.STATE_PROCESSED) for u in Upload.objects.filter(resource=saved_layer)]
                except Exception as e:
                    logger.exception(e)
                    Upload.objects.filter(resource=saved_layer).update(complete=False)
                    [u.set_processing_state(enumerations.STATE_INVALID) for u in Upload.objects.filter(resource=saved_layer)]
                finally:
                    _log(f" -- Upload completed for {saved_layer}... {saved_layer.state}")
