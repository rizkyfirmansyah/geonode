...
urlpatterns = [  # 'geonode.documents.views',
    url(r'^$',
        login_required(documents_list),
        {'facet_type': 'documents'},
        name='document_browse'
        ),
    url(r'^(?P<docid>\d+)/?$',
        login_required(views.document_detail), name='document_detail'),
    url(r'^(?P<docid>\d+)/download/?$',
        login_required(views.document_download), name='document_download'),
    url(r'^(?P<docid>\d+)/link/?$',
        login_required(views.document_link), name='document_link'),
    url(r'^(?P<docid>\d+)/replace$', login_required(DocumentUpdateView.as_view()),
        name="document_replace"),
    url(r'^remove$',
        login_required(views.document_remove), name="document_remove"),
    ...
    url(r'^(?P<docid>[^/]*)/metadata_detail$', login_required(views.document_metadata_detail),
        name='document_metadata_detail'),
    url(r'^(?P<docid>\d+)/metadata$',
        login_required(views.document_metadata), name='document_metadata'),
    url(
        r'^metadata/batch/$',
        login_required(views.document_batch_metadata),
        name='document_batch_metadata'),
    url(r'^(?P<docid>\d+)/metadata_advanced$', login_required(views.document_metadata_advanced),
