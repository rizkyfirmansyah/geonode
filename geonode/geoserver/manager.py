...
class GeoServerResourceManager(ResourceManagerInterface):
        ...
        if instance and isinstance(instance.get_real_instance(), Layer):
            try:
                ...
                if import_session:
                    ...
                    _name = _gs_import_session_info.layer_name if import_session.state == enumerations.STATE_COMPLETE else ''
                    _alternate = f'{_gs_import_session_info.workspace}:{_gs_import_session_info.layer_name}' if import_session.state == enumerations.STATE_COMPLETE else ''
                    _to_update = {
                        'name': _name,
                        'title': instance.title or _gs_import_session_info.layer_name,
                        'files': kwargs.get('files', None),
                        'workspace': _gs_import_session_info.workspace,
                        'alternate': _alternate,
                        'typename': _alternate,
                        'store': _gs_import_session_info.target_store or _gs_import_session_info.layer_name,
                        'subtype': _gs_import_session_info.spatial_files_type.layer_type
                    }
                    if 'defaults' in kwargs:
                        kwargs['defaults'].update(_to_update)
                    Layer.objects.filter(uuid=instance.uuid).update(**_to_update)
    ...
    def _execute_resource_import(self, instance, files: list, user, action_type: str, importer_session_opts: typing.Optional[typing.Dict] = None):
        ...
        _target_store = None
        if gs_layer:
            _target_store = gs_layer.resource.store.name if instance.get_real_instance().subtype == 'vector' else None
        ...
        if not _target_store:
            if instance.get_real_instance().subtype == 'vector' or spatial_files_type.layer_type == 'vector':
        ...
        if _local_files:
            _spatial_files = get_spatial_files_layer_type(ALLOWED_EXTENSIONS, _local_files)
    ...
    def set_permissions(self, uuid: str, /, instance: ResourceBase = None, owner: settings.AUTH_USER_MODEL = None, permissions: dict = {}, created: bool = False,
        ...
        try:
            if _resource:
                ...
                logger.error(f'Fixup GIS Backend Security Rules Accordingly on resource {instance} {isinstance(_resource, Layer)}')
                    ...
                    if settings.OGC_SERVER['default'].get("GEOFENCE_SECURITY_ENABLED", False):
                        if not getattr(settings, 'DELAYED_SECURITY_SIGNALS', False):
                            ...
                            if permissions is not None and len(permissions):
                                if not created:
                                    purge_geofence_layer_rules(_resource)
                                ...
                                if not created:
                                    purge_geofence_layer_rules(_resource.get_self_resource())
                            ...
                            if _disable_cache:
                                ...
                                try:
                                    _layer_workspace = get_layer_workspace(_resource)
                                    toggle_layer_cache(f'{_layer_workspace}:{_resource.name}', filters=filters, formats=formats)
    ...
    def set_thumbnail(self, uuid: str, /, instance: ResourceBase = None, overwrite: bool = True, check_bbox: bool = True) -> bool:
        if instance and (isinstance(instance.get_real_instance(), Layer) or isinstance(instance.get_real_instance(), Map)):
